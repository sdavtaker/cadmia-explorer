...
inline std::vector<std::vector<uint32_t>> build_reverse_adj(const Component &comp) {
...
inline std::vector<bool> compute_ancestors(const Component &comp,
  ...
  int sub =
      (selected_sub >= 0 && selected_sub < static_cast<int>(incoming.size())) ? selected_sub : 0;
...
inline int find_rect_under_cursor(const Component &comp, const Layout &layout, float canvas_ox,
  ...
  for (int i = 0; i < static_cast<int>(comp.rects.size()); ++i) {
    ...
    auto x1 = static_cast<float>(layout.time_to_px(r.time_lo));
    float x2 = x1 + static_cast<float>(layout.rect_pw(r));
    auto y1 = static_cast<float>(layout.out_to_py(r.out_hi)); // top (lower py)
