...
// Drawing helpers
// ---------------------------------------------------------------------------
static void dashed_line(ImDrawList* dl, ImVec2 p0, ImVec2 p1,
                        ...
                        float dash_len = 4.0f, float gap_len = 3.0f)
{
    ...
    while (pos < len) {
        ...
        if (end > len) end = len;
        if (drawing) {
            dl->AddLine({p0.x + nx * pos, p0.y + ny * pos},
...
static void arrow_tip(ImDrawList* dl, ImVec2 tip, ImVec2 from_pt,
                      ImU32 col, float size = 7.0f)
{
    float dx = tip.x - from_pt.x, dy = tip.y - from_pt.y;
    float len = std::sqrt(dx * dx + dy * dy);
    if (len < 0.5f) return;
    float nx = dx / len, ny = dy / len;
    float px = -ny, py = nx;
...
// Rect rendering
// ---------------------------------------------------------------------------
static void draw_rect(ImDrawList* dl, const Rect& r, const Layout& layout,
                      const VP& vp, ImU32 fill_col, ImU32 border_col)
{
    ...
    if (r.multiplicity > 1) {
        char buf[32];
        std::snprintf(buf, sizeof(buf), "\xc3\x97%u", r.multiplicity); // ×N
        ImVec2 tsz = ImGui::CalcTextSize(buf);
        dl->AddText({x2 - tsz.x - 2.0f, y1 + 1.0f}, border_col, buf);
    }
}
...
// Edge rendering
// ---------------------------------------------------------------------------
static void draw_edges(ImDrawList* dl, const Component& comp,
                       const Layout& layout, const VP& vp)
{
...
// Axis rendering
// ---------------------------------------------------------------------------
static void draw_axes(ImDrawList* dl, const Component& comp,
                      const Layout& layout, const VP& vp)
{
    ...
    for (int i = 0; i <= N_TICKS; ++i) {
        ...
        ImVec2 tsz = ImGui::CalcTextSize(buf);
        dl->AddText({tx - tsz.x * 0.5f, ax_y + 7.0f}, text_col, buf);
    }
    ...
    for (double y : y_vals) {
        ...
        const char* lbl = "";
        for (const auto& r : comp.rects) {
            if (r.out_lo == y) { lbl = r.out_label.c_str(); break; }
        }
...
void render_component(ImDrawList* dl,
                      ...
                      ImVec2 canvas_size)
{
    VP vp{canvas_pos.x, canvas_pos.y,
