...
std::string svg_line(double x1, double y1, double x2, double y2,
    ...
    std::string s = fmt(
        R"SVG(<line x1="%.2f" y1="%.2f" x2="%.2f" y2="%.2f" stroke="%s" stroke-width="%.1f")SVG",
...
std::string draw_rect_sides(double x1, double y1, double x2, double y2,
                            bool left_solid, bool right_solid,
                            bool top_solid,  bool bottom_solid,
                            const char* stroke = "#2060a0",
...
std::vector<double> axis_ticks(double lo, double hi, int n_ticks) {
    ...
    if (hi <= lo || n_ticks < 2) return ticks;
...
std::string render_svg(const Component& comp, const Layout& L) {
    ...
    // Canvas background
    out << fmt(
        R"SVG(<rect width="%.0f" height="%.0f" fill="#fafafa"/>)SVG" "\n",
        Layout::CANVAS_W, Layout::CANVAS_H);
    ...
    for (const auto& rect : comp.rects) {
        ...
        out << draw_rect_sides(x1, y1, x2, y2,
                               rect.time_lo_closed, rect.time_hi_closed,
    ...
    // X axis
    {
        ...
        out << svg_line(Layout::PLOT_X, ax_y,
                        Layout::PLOT_X + Layout::PLOT_W, ax_y, "#333", 1.0);
        for (double t : axis_ticks(L.t_min, L.t_max, 8)) {
            double px = L.time_to_px(t);
            if (px < Layout::PLOT_X - 1 || px > Layout::PLOT_X + Layout::PLOT_W + 1) continue;
            out << svg_line(px, ax_y, px, ax_y + 5, "#333", 1.0);
            out << svg_text(px, ax_y + 17, fmt_time(t));
        }
        out << svg_text(Layout::PLOT_X + Layout::PLOT_W / 2.0,
                        ax_y + 40, "simulation time", "middle", 11.0, "#555");
    }

    // Y axis
    {
        double ax_x = Layout::PLOT_X;
        out << svg_line(ax_x, Layout::PLOT_Y,
                        ax_x, Layout::PLOT_Y + Layout::PLOT_H, "#333", 1.0);
