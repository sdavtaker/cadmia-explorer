...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--all") {
      ...
      if (i + 1 >= argc) {
        std::cerr << "--component requires an argument\n";
        return 1;
      }
      ...
      if (i + 1 >= argc) {
        std::cerr << "--output-dir requires an argument\n";
        return 1;
      }
      output_dir = argv[++i];
      headless_mode = true;
  ...
  for (const auto &comp : data.components) {
    ...
    for (char c : comp.name) {
      safe_name +=
          ((std::isalnum(static_cast<unsigned char>(c)) != 0) || c == '-' || c == '_') ? c : '_';
    }
    ...
    f << svg;
    std::cout << "  Wrote " << out_path << " (" << comp.rects.size() << " rects, "
              << comp.edges.size() << " edges)\n";
    ++rendered;
  }
