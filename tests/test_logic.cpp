...
static Rect make_rect(double t0, double t1, double y0, double y1) {
    Rect r{};
    r.time_lo = t0; r.time_hi = t1;
    r.out_lo  = y0; r.out_hi  = y1;
...
TEST_CASE("find_rect_under_cursor: no rects") {
    Component c;
    Layout layout{};
    int hit = find_rect_under_cursor(c, layout,
        0.0f, 0.0f, (float)Layout::CANVAS_W, (float)Layout::CANVAS_H,
        0.0f, 0.0f, 1.0f, 100.0f, 100.0f);
    REQUIRE(hit == -1);
}
...
TEST_CASE("find_rect_under_cursor: click outside all rects") {
    ...
    int hit = find_rect_under_cursor(c, layout,
        0.0f, 0.0f, (float)Layout::CANVAS_W, (float)Layout::CANVAS_H,
        0.0f, 0.0f, 1.0f, 2.0f, 450.0f); // deep in left margin
    REQUIRE(hit == -1);
}
...
TEST_CASE("find_rect_under_cursor: last overlapping rect wins") {
    ...
    int hit = find_rect_under_cursor(c, layout,
        0.0f, 0.0f, (float)Layout::CANVAS_W, (float)Layout::CANVAS_H,
        0.0f, 0.0f, 1.0f, (float)cx_l, (float)cy_l);
